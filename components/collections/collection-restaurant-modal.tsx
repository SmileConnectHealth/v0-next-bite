...
import { X, MapPin, Phone, Globe, Plus } from "lucide-react"
import { Dialog, DialogContent } from "@/components/ui/dialog"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Button } from "@/components/ui/button"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Badge } from "@/components/ui/badge"
import AddToCollectionModal from "../discover/add-to-collection-modal"
import RatingModal from "../discover/rating-modal"
...
export default function RestaurantModal({ restaurant, isOpen, onClose }: RestaurantModalProps) {
  ...
  const [restaurantReviews, setRestaurantReviews] = useState([
    ...
    },
    {
      ...
      rating: 8.5,
      date: "1 month ago",
      text: "Really enjoyed my meal here. The atmosphere was great and the food was delicious. Only giving 8.5 because it was a bit noisy.",
  ...
  const handleRatingSubmit = (rating: number, reviewText: string) => {
    const newReview = {
      id: Date.now(),
      user: { name: "You", image: "/placeholder.svg?height=100&width=100" },
      rating: rating,
      date: "Just now",
      text: reviewText,
      tags: [],
        ...
        <div className="p-6 overflow-y-auto flex-1">
          <div className="mb-6">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center">
                <svg className="h-5 w-5 mr-1 text-yellow-400 fill-yellow-400" viewBox="0 0 24 24">
                ...
                <span className="text-sm text-gray-400 ml-1">({restaurant.ratings || 0})</span>
              </div>
            </div>
          ...
          <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="text-gray-300">
            ...
            <TabsContent value="overview" className="space-y-6">
              ...
              {restaurant.notes && (
                <div>
                  <h3 className="font-medium text-white mb-2">Collection Notes</h3>
                  <div className="bg-[#2A3542] p-3 rounded-md text-gray-300 italic">
                    <p>{restaurant.notes}</p>
                    <div className="text-right text-xs text-gray-400 mt-2">Added by {restaurant.addedBy}</div>
                  </div>
                </div>
              )}
            </TabsContent>
